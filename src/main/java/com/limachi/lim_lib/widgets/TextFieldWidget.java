...
public class TextFieldWidget extends BaseWidget<TextFieldWidget> {
    ...
    protected final ArrayList<Pair<Integer, String>> history = new ArrayList<>();
    ...
    public TextFieldWidget pushToHistory() {
        history.add(++historyPage, new Pair<>(cursor, text));
    ...
    public TextFieldWidget loadFromHistory(int depth) {
        historyPage = Mth.clamp(historyPage - depth, 0, history.size() - 1);
        Pair<Integer, String> p = history.get(historyPage);
