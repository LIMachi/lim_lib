...
public class NBT {
    ...
    protected static CompoundTag serializeAnnotations(Class<?> clazz, Object object, CompoundTag store) {
        ...
        for (Field field : fields) {
            ...
            if (a != null) {
                ...
                if (fv != null) {
                    if (field.getGenericType() instanceof ParameterizedType pt && (Collection.class.isAssignableFrom(field.getType()) || Map.class.isAssignableFrom(field.getType()))) {
        ...
        for (Field field : fields) {
            ...
            if (a != null) {
                ...
                if (field.getGenericType() instanceof ParameterizedType pt && (Collection.class.isAssignableFrom(field.getType()) || Map.class.isAssignableFrom(field.getType()))) {
                    ...
                    out = deserializeFromTag(field.getType(), tag.get(name));
                try {
    ...
    public static <T> T deserializeFromTag(Class<T> clazz, Object object, Tag tag) {
        if (tag == null)
            ...
            return (T)tag;
        else if (Boolean.class.isAssignableFrom(clazz) && tag instanceof ByteTag t)
            return (T)(Boolean)t.equals(ByteTag.ONE);
        else if (Byte.class.isAssignableFrom(clazz) && tag instanceof ByteTag t)
            return (T)(Byte)t.getAsByte();
        else if (Character.class.isAssignableFrom(clazz) && tag instanceof IntTag t)
            return (T)(Integer)t.getAsInt();
        else if (Short.class.isAssignableFrom(clazz) && tag instanceof ShortTag t)
            return (T)(Short)t.getAsShort();
        else if (Integer.class.isAssignableFrom(clazz) && tag instanceof IntTag t)
            return (T)(Integer)t.getAsInt();
        else if (Long.class.isAssignableFrom(clazz) && tag instanceof LongTag t)
            return (T)(Long)t.getAsLong();
        else if (Float.class.isAssignableFrom(clazz) && tag instanceof FloatTag t)
            return (T)(Float)t.getAsFloat();
        else if (Double.class.isAssignableFrom(clazz) && tag instanceof DoubleTag t)
