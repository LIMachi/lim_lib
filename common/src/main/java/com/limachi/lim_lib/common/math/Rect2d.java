...
public record Rect2d(double x, double y, double w, double h) implements Constable {
    ...
    public static Rect2d from(ScreenRectangle screenRect) { return new Rect2d(screenRect.left(), screenRect.top(), screenRect.width(), screenRect.height()); }
    @Environment(EnvType.CLIENT)
    public ScreenRectangle screenRectangle() { return new ScreenRectangle((int)x, (int)y, (int)w, (int)h); }
