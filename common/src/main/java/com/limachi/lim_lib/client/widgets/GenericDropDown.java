...
public class GenericDropDown extends AbstractContainerWidget {
    ...
    public void updateVisibility() {
        if (expanded)
            setHeight(slotHeight * Math.min(list.size(), maxVisible));
        else
            setHeight(slotHeight);
        for (int i = 0; i < list.size(); ++i) {
            var w = list.get(i);
            w.setWidth(width - (expanded && list.size() > maxVisible ? scrollBarWidth : 0));
            w.setHeight(slotHeight);
            w.setX(getX());
            w.setY(getY() + i * slotHeight - scroll);
    ...
    protected void renderWidget(GuiGraphics guiGraphics, int i, int j, float f) {
        if (expanded) {
            ...
            RenderSystem.enableScissor((int)(getX() * window.getGuiScale()), (int)(window.getHeight() - (getY() + slotHeight * maxVisible) * window.getGuiScale()), (int)(getWidth() * window.getGuiScale()), (int)(slotHeight * maxVisible * window.getGuiScale()));
            for (var w : list)
                w.render(guiGraphics, i, j, f);
            if (list.size() > maxVisible) {
                int x = getX() + getWidth() - scrollBarWidth;
                double height = slotHeight * (double)maxVisible;
                guiGraphics.fill(x, getY(), x + scrollBarWidth, getY() + (int)height, 0xFF555555);

                double ratio = list.size() / (double)maxVisible;
                double pixelRatio = slotHeight / ratio;
                double knob = maxVisible * (slotHeight / ratio);

                double y = getY() + scroll / pixelRatio;
                guiGraphics.fill(x, (int)y + 1, x + scrollBarWidth, (int)(y + knob + 1), 0xFFAAAAAA);
            }
